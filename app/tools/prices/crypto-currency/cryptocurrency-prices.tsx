...
export default function CryptocurrencyPrices() {
  ...
  const fetchPrices = async (isRefresh = false) => {
    try {
      ...
      const filteredCryptocurrencies = cryptoSymbols
        ...
        .filter(
          (item): item is CryptocurrencyItem => Boolean(item)
        );
      ...
      setError(
        "دریافت اطلاعات ارزهای دیجیتال با مشکل مواجه شد."
      );
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const bitcoin = cryptocurrencies.find(
    (item) => item.symbol === "BTC"
  );

  const ethereum = cryptocurrencies.find(
