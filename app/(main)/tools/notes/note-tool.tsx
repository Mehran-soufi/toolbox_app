  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      try {
        ...
        if (savedNotes) {
          const parsedNotes = JSON.parse(savedNotes) as unknown;
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-zinc-500 dark:text-zinc-400">
          ...
          {activeNoteId && (
            <span>
              ...
              آخرین ویرایش:{" "}
              {formatDate(
                notes.find((note) => note.id === activeNoteId)?.updatedAt ??
        ...
        <div className="mt-5 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-4">
          ...
          >
            ...
            {copied ? (
              ...
              <>
                {" "}
                <Clipboard className="size-4" /> کپی متن{" "}
              </>
            )}{" "}
          </Button>{" "}
          <Button
          ...
          >
            {" "}
            <Eraser className="size-4" /> پاک کردن{" "}
          </Button>{" "}
          <Button
            type="button"
            variant="outline"
          ...
          >
            {" "}
            <FilePlus className="size-4" /> یادداشت جدید{" "}
          </Button>{" "}
        </div>{" "}
      </div>{" "}
      {notes.length > 0 && (
        <div className="w-full rounded-2xl border border-zinc-200/70 bg-white/60 p-5 shadow-sm backdrop-blur-xl dark:border-zinc-800 dark:bg-zinc-900/50">
          ...
          <div className="space-y-3">
            ...
            {notes.map((note) => (
              <div
                key={note.id}
                className={`rounded-xl border p-4 transition-colors ${activeNoteId === note.id ? "border-violet-500/30 bg-violet-500/5" : "border-zinc-200 bg-zinc-50/50 dark:border-zinc-800 dark:bg-zinc-950/30"}`}
              >
                {" "}
                <button
                  type="button"
